...
def main(test_val, method='knn'):
    ...
    if method == "knn":
        ...
        model = LR()
    test_val = [float(i) for i in test_val.strip('[]').split(',')]
    test_val = np.array(test_val).reshape(1, -1)
    return model, test_val

if __name__=="__main__":
    model, test_val = main(method=sys.argv[1], test_val=sys.argv[2])
    ...
    print("Probabiliites: {}".format(probabilities))
